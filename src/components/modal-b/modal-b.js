...
class ModalB extends Component {
    ...
    async getSearchData() {
        let url = `https://api.dev.pastorsline.com/api/contacts.json?companyId=171&countryId=226&query=${this.state.searchQuery}`;
        let response = await axios.get(url, {
            headers: {
                Authorization: Config.token
            }
    ...
    render() {
        return (
            <>
                            ...
                            <div className="modal-body">
                                ...
                                <div className='position-relative d-flex flex-column p-3'>
                                            ...
                                            this.state.contactsDOM.map((obj, index) => {
                                                return (
                                                    <div key={index} className='border contact-card m-1 p-1'
                                                         ...
                                                         data-toggle="modal" data-target="#contact-modal-b">
                                                        <div>Id: {obj.id}</div>
                                    ...
                                    {this.state.isDataFetching ? <div className='loading'><i
                                        className="fa fa-circle-o-notch fa-spin" style={{'fontSize': '30px'}}/>
                                    </div> : null}
                                </div>
                ...
                <div className="modal fade" id="contact-modal-b" tabIndex="-1" role="dialog"
